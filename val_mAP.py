...
def run(
    ...
    if not training:
        if pt and not single_cls:  # check --weights are trained on --data
            ...
            assert ncm == nc, f'{weights[0]} ({ncm} classes) trained on different --data than what you passed ({nc} ' \
                              f'classes). Pass correct combination of --weigWhts and --data that are trained together.'
    ...
    if save_json and len(jdict):
        ...
        anno_json = str('instances_test2017.json')  # annotations json
        ...
        try:
            ...
            eval.accumulate()
            eval.summarize()
            map, map75, map50 = eval.stats[:3]  # update results (mAP@0.5:0.95, mAP@0.5)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/infraredImg.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default='./weights/ISE-YOLO-S.pt', help='model.pt path(s)')
