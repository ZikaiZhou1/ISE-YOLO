...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        p = str(Path(path).resolve())  # os-agnostic absolute path 转换为绝对路径
        if '*' in p:    # 有没有带星
            files = sorted(glob.glob(p, recursive=True))  # glob
        elif os.path.isdir(p):  # 是不是一个文件夹
            files = sorted(glob.glob(os.path.join(p, '*.*')))  # dir
        elif os.path.isfile(p): # 是不是一个文件
            files = [p]  # files
        else:
            raise Exception(f'ERROR: {p} does not exist')

        images = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS]  # 查看是不是可用的图片格式
        videos = [x for x in files if x.split('.')[-1].lower() in VID_FORMATS]  # ……视频……
    ...
    def __next__(self):
        ...
        return path, img, img0, self.cap, s # img:resize后的图片；img0：原图；cap：none；s：打印信息
